...
class EvaluationService:
    ...
    def __init__(
        ...
        Args:
            metric_registry (Optional[MetricRegistry]): Registry for metric providers.
            threshold_manager (Optional[ThresholdManager]): Threshold gate manager.
            score_calculator (Optional[ScoreCalculator]): Weighted score calculator.
            history_manager (Optional[EvaluationHistoryManager]): Run history manager.
            report_generator (Optional[ReportGenerator]): Report generator.
            pdf_exporter (Optional[PDFExporter]): PDF exporter.
            json_exporter (Optional[JSONExporter]): JSON exporter.
        """
        # 1. Setup Metric Registry and register default providers
        self.metric_registry = metric_registry or MetricRegistry()
        self._register_default_providers()

        # 2. Setup Threshold Manager
        self.threshold_manager = threshold_manager or ThresholdManager()

        # 3. Setup Score Calculator with default weighting
        default_weight_config = WeightingEngine.get_preset_config(
            WeightingEngine.get_preset_config.__defaults__[0] if hasattr(WeightingEngine.get_preset_config, "__defaults__") else WeightingEngine.get_preset_config
        ) if False else WeightConfig()
        self.score_calculator = score_calculator or ScoreCalculator(default_weight_config)

        # 4. Setup History, Reports, and Exporters
